...
public class Util 
{
	...
	public static String makeFormatStringPart(List<Part> parts)
	{
		...
		for (Part part : parts)
		{
			...
			{
				tb.append("[");
				tb.append(makeFormatStringPart(((ForPart)part).getModelBlock().getParts()));
				tb.append("]");
			}
			else if (part instanceof SwitchPart)
			{
				...
				while (i.hasNext())
				{
					...
					tb.append(makeFormatStringPart(entry.getParts()));
					if(i.hasNext())
						tb.append("|");
				}
				tb.append("]");
	...
	public static CharSequence makeFormatStringPartType(List<PartType> idParts)
	{
		TextBuilder tb = new TextBuilder();
